...
const reportHotel = [
  {
    ...
    bookingID: "B123",
    passengerName: "John report hotel",
...
export async function GET(req:Request) {
  ...
  const paginatedData = reportHotel.slice(startIndex, endIndex);

  const totalPages = Math.ceil(reportHotel.length / limit);

  return NextResponse.json({
    page,
    limit,
    totalPages,
    totalItems: reportHotel.length,
    data: paginatedData,
  });
}
