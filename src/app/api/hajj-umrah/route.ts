...
export async function GET(req:Request) {
  ...
  const paginatedData = hajjUmrah.slice(startIndex, endIndex);

  const totalPages = Math.ceil(hajjUmrah.length / limit);

  return NextResponse.json({
    page,
    limit,
    totalPages,
    totalItems: hajjUmrah.length,
    data: paginatedData,
  });
}
