...
const refundHotel = [
  {
    ...
    bookingID: "B123",
    passengerName: "John Doe r hotel",
...
export async function GET(req:Request) {
  ...
  const paginatedData = refundHotel.slice(startIndex, endIndex);

  const totalPages = Math.ceil(refundHotel.length / limit);

  return NextResponse.json({
    page,
    limit,
    totalPages,
    totalItems: refundHotel.length,
    data: paginatedData,
  });
}
