...
export async function GET(req:Request) {
  ...
  const paginatedData = refundHajj.slice(startIndex, endIndex);

  const totalPages = Math.ceil(refundHajj.length / limit);

  return NextResponse.json({
    page,
    limit,
    totalPages,
    totalItems: refundHajj.length,
    data: paginatedData,
  });
}
