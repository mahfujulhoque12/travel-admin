...
const reportPackage = [
  {
    ...
    bookingID: "B123",
    passengerName: "John report package",
...
export async function GET(req:Request) {
  ...
  const paginatedData = reportPackage.slice(startIndex, endIndex);

  const totalPages = Math.ceil(reportPackage.length / limit);

  return NextResponse.json({
    page,
    limit,
    totalPages,
    totalItems: reportPackage.length,
    data: paginatedData,
  });
}
