...
export async function GET(req:Request) {
  ...
  const paginatedData = refundCar.slice(startIndex, endIndex);

  const totalPages = Math.ceil(refundCar.length / limit);

  return NextResponse.json({
    page,
    limit,
    totalPages,
    totalItems: refundCar.length,
    data: paginatedData,
  });
}
