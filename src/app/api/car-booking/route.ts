...
export async function GET(req:Request) {
  ...
  const paginatedData = carBooking.slice(startIndex, endIndex);

  const totalPages = Math.ceil(carBooking.length / limit);

  return NextResponse.json({
    page,
    limit,
    totalPages,
    totalItems: carBooking.length,
    data: paginatedData,
  });
}
