...
export async function GET(req:Request) {
  ...
  const paginatedData = refundFlight.slice(startIndex, endIndex);

  const totalPages = Math.ceil(refundFlight.length / limit);

  return NextResponse.json({
    page,
    limit,
    totalPages,
    totalItems: refundFlight.length,
    data: paginatedData,
  });
}
