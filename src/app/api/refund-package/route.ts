...
export async function GET(req:Request) {
  ...
  const paginatedData = refundPackage.slice(startIndex, endIndex);

  const totalPages = Math.ceil(refundPackage.length / limit);

  return NextResponse.json({
    page,
    limit,
    totalPages,
    totalItems: refundPackage.length,
    data: paginatedData,
  });
}
