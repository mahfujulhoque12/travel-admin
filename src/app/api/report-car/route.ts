...
export async function GET(req:Request) {
  ...
  const paginatedData = reportCar.slice(startIndex, endIndex);

  const totalPages = Math.ceil(reportCar.length / limit);

  return NextResponse.json({
    page,
    limit,
    totalPages,
    totalItems: reportCar.length,
    data: paginatedData,
  });
}
